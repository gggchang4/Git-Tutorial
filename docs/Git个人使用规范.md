...
这一章把前面的分支、提交、发布和协作规则整理成三套场景化 SOP。它不是断言所有团队都完全一致，而是按协作复杂度给出一套“默认闭环”：个人项目可以直接照做，小团队可以轻量落地，大公司场景则用于提前训练工程化协作意识；真正落地时，始终以目标仓库和团队规则为准。

### 1. 使用方式

- `硬性约束`：先判断自己当前处于个人仓库、小团队仓库，还是受公司流程约束的仓库，再套用对应 SOP
- `硬性约束`：只要目标仓库已经存在分支保护、CI、代码所有者或发布规则，就优先遵循仓库规则
- `推荐做法`：把 SOP 理解为“从接到任务到安全合并”的最小闭环，而不是只记几个命令
- `推荐做法`：执行 SOP 时联动本手册里的分支、提交、冲突和配套规范，不割裂使用

### 2. 三种场景的最小差异

| 场景 | 核心目标 | 必经关卡 | 不该省略的动作 |
|------|------|------|------|
| 个人开发 | 保持历史清晰、能稳定发布 | 自检通过、分支合回、标签可追溯 | `git status`、`git diff --cached`、最小验证 |
| 小团队开发 | 保持协作透明、降低互相踩踏 | PR / MR、review、基础 CI | 需求对齐、PR 说明、合并后删分支 |
| 大公司开发 | 保持可审计、可回滚、可规模化协作 | 工单关联、受保护分支、自动化检查、多角色审批 | 风险说明、验证记录、回滚路径、发布同步 |

### 3. 个人开发 SOP

适用场景：个人作品、学习项目、工具仓库，或者暂时没有强制 review 流程的仓库。
...
1. 开始前先把本次改动目标写进 `TODO.md`、issue 或自己的任务清单，避免“顺手改一堆”。
2. 从 `develop` 拉日常开发分支；只有正式发布和紧急修复才碰 `main`。
3. 开发过程中保持小步提交，让每个提交都能单独解释。
4. 提交前至少完成 `git status`、`git diff --cached` 和与你改动强相关的最小验证。
5. 合回后及时删除已完成分支，准备下一轮开发。

这套 SOP 的重点不是“模仿团队感”，而是让个人项目也具备可回看、可发布、可维护的节奏。

### 4. 小团队开发 SOP

适用场景：2 到 5 人左右协作仓库，已经开始依赖 PR / MR、基础 CI 和简单分工。
...
1. 开工前先对齐目标、边界、影响范围和目标分支，避免多人同时改同一块而彼此不知情。
2. 日常功能和常规修复优先合向 `develop`，不要直接把未审查改动推到 `main`。
3. PR / MR 说明至少写清楚三件事：改了什么、怎么验证、还有什么风险或待确认项。
4. review 关注的不只是代码风格，还包括分支是否干净、提交是否可读、测试和文档是否跟上。
5. 合并后同步删除分支，并把 issue、任务卡或发布记录更新到最新状态。

这套 SOP 的重点是让小团队在不引入过重流程的前提下，先把“透明协作”和“可 review”建立起来。

### 5. 大公司开发 SOP

适用场景：存在明确工单系统、受保护分支、自动化检查、代码所有者或发布审批的团队。

这里写的是基于公开实践整理出的通用闭环，不等同于任何一家公司的完整内部制度。
...
-> 开发并补齐测试 / 文档 / 配置变更说明
-> 发起 PR 并关联工单
-> 等待 CI / 静态检查 / owner review / 必要审批
-> 按分支保护或发布队列合并
-> 合并后观察、回写记录、必要时走 hotfix
```

执行要点：

1. 先从工单、需求卡或缺陷单开始，而不是直接开改；确保需求归属、优先级和目标版本明确。
2. 分支命名、提交格式和目标分支通常受团队规则约束，个人习惯必须服从仓库规则。
3. 除了代码本身，通常还要同步补齐测试、迁移脚本、配置说明、回滚方案或监控影响说明。
4. PR 不只是“请求合并”，还承担审计入口作用，最好把变更背景、验证记录、风险点和回滚路径写完整。
5. 合并动作往往不能手工随意完成，而要经过受保护分支、合并队列、发布列车或值班发布流程。
6. 如果线上有问题，优先走团队既定的 `hotfix`、回滚或发布审批流程，而不是个人临时拍板。

这套 SOP 的重点是让改动具备可审计、可验证、可回滚能力。对个人来说，提前训练这些意识，比死记某家公司的细节流程更重要。
